...
int main(int argc, char *argv[]) {
  ...
  if (!HasMatchingFileExtension(output_file_name, image_format)) {
    std::cout << output_file_name
              << " is missing the required file extension .png.\n";
  }
  Magick::ColorRGB white(1, 1, 1);
  Magick::Image image(Magick::Geometry(kImageWidth, kImageHeight), white);

  for (int column = (static_cast<int>(image.columns()) - 1); column >= 0;
       column--) {
    for (int row = 0; row < image.rows(); row++) {
      double red = (row / (static_cast<double>(image.columns()) - 1.0));
      double green = (column / (static_cast<double>(image.rows()) - 1));
      double blue(0.25);
